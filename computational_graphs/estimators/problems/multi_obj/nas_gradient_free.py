...
class NasBench201GradientFree(Problem):
    def __init__(self, 
                dataset, 
                max_nodes, 
                sample_batch=100,
        ...
        self.n_bits_per_op = int(np.floor(np.log2(len(ops.NAS_BENCH_201))) + 1)
        super().__init__(n_params=self.n_bits_per_op * (max_nodes * (max_nodes-1)) // 2,
                        n_obj=3,
                        constraints=0,
                        type=np.int,
                        **kwargs)
        ...
        self.lrc_model = Linear_Region_Collector(
                          ...
                          data_path='~/.torch/',
                          seed=1)
        self.loader = self.lrc_model.train_loader
        # self.global_ntk = []
        # self.global_lrc = []
    ...
    def _f(self, x):
        ...
        phenotype = self.__decode(genotype)
        index = self.api.query_index_by_arch(phenotype)
        f1_x = self.api.get_cost_info(index, self.dataset)['flops']
        config = self.api.get_net_config(index, self.dataset)
        network = get_cell_based_tiny_net(edict(config))
        
        network.to(self.device)
        f2_x = self.__calc_ntk(network)
        f3_x = self.__calc_lrc(network)

        self.logger.info('arch: {} - flops: {} - ntk: {} - lrc: {}'.format(genotype, f1_x, f2_x, f3_x))
        f_x += [f1_x, f2_x, f3_x]
