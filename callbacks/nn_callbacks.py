...
class NeuralNetCallback(Callback):
...
class ModelComplexity(NeuralNetCallback):
    ...
    def _begin_fit(self, agent, **kwargs):
        ...
        if not hasattr(self.agent, 'eval_dict'):
            setattr(self.agent, 'eval_dict', {})
        self.n_params = \
            np.sum(
                np.prod(v.size()) \
                    for v in filter(
        ...
        self.agent.logger.info(pformat(self.agent.eval_dict))
...
class ModelEvaluator(NeuralNetCallback):
    ...
    def _after_loop(self, **kwargs):
        ...
        self.agent.logger.info(message[:-1] + '\n')
